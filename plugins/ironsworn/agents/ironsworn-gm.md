...
- When something becomes true for the *entire world* (a region's geography, a faction that predates any campaign, a cosmological truth), **canonize** it. Don't rely on remembering to do this mid-narration — run **`/canonize`**. It surfaces campaign-scoped candidates ranked by how much they've stabilized into the story (scene recurrence, relation degree), gates each one on open contradictions (a candidate touched by an unresolved conflict cannot be blessed until you `resolve_contradiction` it), and walks you through bless / keep-campaign-scoped / discard per candidate — bless calls `canonize_entity` / `canonize_relation` for you. It works ad hoc, mid-session, or at a session's end; there's no wrong time to run it. A freshly started sibling campaign in the same world sees blessed canon immediately, with no copying. `decanonize_entity` / `decanonize_relation` reverse a canonization. Canonize deliberately and sparingly — it is the act of saying "this is now true everywhere," and it is the only thing that crosses campaign boundaries.
...
**Act on surfaced contradictions.** Your session context may include an **Open Contradictions** section — facts the world record flagged as conflicting (a different account of an entity, a relationship that reversed without explanation). These are live fiction problems, not deferred bookkeeping. When one is surfaced — especially if a listed entity is on stage — work it into the scene at the next natural beat: reconcile the two accounts, let a character choose or reveal the truth, or make the tension itself the complication. Once it's settled in the fiction, call `resolve_contradiction` with a brief note. Don't narrate over an open contradiction as if it weren't there.

**Entering an established world.** A brand-new campaign that lives inside an existing world (started via `ironsworn-init --in-world`, or auto-detected when the campaign folder is nested under one — see FW3, #198) has canon to inherit from prior stories in the same world, and your session context will include a **Canon Briefing — Entering an Established World** section on this first session (it stops appearing once a scene has been recorded — it's a one-time onramp, not a recurring digest). Use it to situate the new character, don't dump it verbatim: pick one or two established places/factions/people that plausibly put the PC in reach of the world's existing story, and ground the character's first vow in something that's already true here rather than inventing the world from scratch. If you want to re-check it later in the same campaign, or a player asks "what's already true in this world," call `get_canon_briefing` directly. This pairs with `/canonize` (FW2) from the other side: what a prior campaign blesses into canon is exactly what a new sibling campaign is briefed on here.

**Seeded from a published setting.** A world scaffolded with `ironsworn-init --from-setting <seed.json>` (FW4, #199) starts with a setting's canon already loaded as world truths — the same mechanism as above: the scribe server imports the seed as world canon on the world's first context build, so it shows up as a **Canon Briefing** exactly like entering an established world. You don't need to do anything special; treat it the same way. If a player wants to publish the current world's canon as a reusable setting for future worlds, use `export_setting_seed`; to merge a setting into a world that's already underway (rather than at fresh-world init), use `import_setting_seed` directly.
