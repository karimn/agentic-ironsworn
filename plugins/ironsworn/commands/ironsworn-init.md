---
description: Scaffold the current folder as an Ironsworn campaign, or start a new campaign in an existing world (safe to re-run — never overwrites existing files)
argument-hint: "[--in-world <path-to-existing-world-root>] [campaign-id] [campaign-name]"
allowed-tools: ["Bash"]
---
...
Show the script output verbatim to the user without summarizing it. If the script exits non-zero (e.g. `--in-world` pointed at a path that isn't an existing world root), surface the error message plainly rather than retrying with guessed arguments.
