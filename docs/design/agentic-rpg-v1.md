...
| **FW4** ✅ | **Setting-seed unbuilt** — `world-truths` Q&A establishes truths for a *new* world ✅, but inheriting a *published* setting's canon (migration step #4) is not started. Straddles platform; the fiction-facing half is the undone part. **Done:** `export_setting_seed`/`import_setting_seed` MCP tools (`packages/core/src/rag/setting-seed.ts`) round-trip a world's `campaign_id IS NULL` canon as portable JSON; `ironsworn-init.sh --from-setting <seed.json>` stages it at world-init, and `buildContext` auto-imports it on the world's first session, reusing FW3's Canon Briefing to present it — no npm setting-package distribution (deferred to platform, #7). | #199 |
| **FW5** ✅ | **Narration quality is unmeasured** (frontier) — the eval measures *capture* (does the KG reflect play); nothing measures whether grounding makes narration *better*. Scope as a research spike, don't block v1 on it. **Done:** spike doc (`docs/spikes/2026-07-narration-quality.md`) concludes *partial yes* — the canon-consistency floor is cheaply measurable by reusing `checkEntityContradiction`/`checkRelationContradiction` as an eval scorer (proposed as a fast-follow, not built here); specificity/continuity are graded aesthetic judgments not worth an LLM-judge harness yet, deferred to a human rubric checklist during play. | #200 |
